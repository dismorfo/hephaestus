...
function html (data) {
  ...
  try {
    ...
    if (agartha.exists(agartha.path.join(agartha.appDir(), 'content.json'))) {
      _.extend(source.content, agartha.read.json(agartha.path.join(agartha.appDir(), 'content.json')))
    }
    ...
    const app_partials_dir = agartha.path.join(agartha.appDir(), 'app/partials')
    const app_task_partials_dir = agartha.path.join(agartha.appDir(), 'app/pages', task)
    if (agartha.exists(app_partials_dir)) {
      _.each(agartha.readdirSync(app_partials_dir), (filename) => {
        if (!filename.match('.hbs')) return
        let name = agartha.path.basename(filename, '.hbs')
        let partial = null
        // we found a unregistered partial inside {appRoot}/app/partials
        // figure out if this task overwrites it
        const app_task_partials_dir_partial = agartha.path.join(app_task_partials_dir, filename)
        // task overwrite app partial, use partial located inside {appRoot}/app/pages/{task}
        if (agartha.exists(app_task_partials_dir_partial)) {
          partial = agartha.read.text(app_task_partials_dir_partial)
        }
        // use app partial located inside {appRoot}/app/partials
        else {
          partial = agartha.read.text(agartha.path.join(app_partials_dir, filename))
        }
        /** register Handlebars partials */
        Handlebars.registerPartial(name, partial)        
      })
        ...
        _.each(data.assets.js, (js) => {
          ...
          if (type != 'cdn') {
            if (!agartha.exists(agartha.path.join(agartha.appBuildDir(), 'js', basename))) {
              ...
              let jsFile = agartha.path.join(agartha.appBuildDir(), 'js', basename)
              if (!agartha.exists(jsFile)) {
                if (!_.has(agartha._filemap, jsFile)) {
                  agartha._filemap[jsFile] = true
                  agartha.write(jsFile, code)
                }
              }
            }
          }
    ...
    // write HTML file
    agartha.write(
      agartha.path.join(agartha.appBuildDir(), route),
      htmlminify(handlebars_template(source), htmlminifyConfiguration)
    )
    // agartha.write(agartha.path.join(agartha.appBuildDir(), task + '.json'), JSON.stringify(source, null, 4))
  }
  catch (error) {
    console.error(error)
  }
...
function templateToUse (source) {
  ...
  // 1) Current module      
  template = agartha.path.join(source.current, baseTemplateFilename)
  if (agartha.exists(template)) {
    return template
  }

  // 2) Current site shared resources
  template = agartha.path.join(appDir, 'app/partials', baseTemplateFilename)
  if (agartha.exists(template)) {
    return template
  }
  
  // 3) Base module
  template = agartha.path.join(source.base, baseTemplateFilename)
  if (agartha.exists(template)) {
    return template
  }

  // 4) Relic shared resources
  template = agartha.path.join(agartha.relicDir(), 'partials', baseTemplateFilename)
  if (agartha.exists(template)) {
    return template
  }
