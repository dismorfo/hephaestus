...
async function transform (raw_code = '') {
  try {
    return transformSync(raw_code, configurations.babel());
  } catch (error) {
    console.log(error);
    process.exit(1);
  }
...
async function asyncCall (raw_code = []) {
  let out = '';
  for (let entry of raw_code) {
    const code = await transform(entry);
    out += code.code;
  }
  return out;
}
...
async function es5 (js, module) {
  ...
  // 2) Check if we have an external URL
    ...
    const closure = await _.template(template.external)({ src: js, now: timespan });
    return closure;
  }
