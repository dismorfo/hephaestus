  ...
  try {
    ...
    if (_.isObject(data.assets)) {
      ...
      if (_.isArray(data.assets.js)) {
        /**
         *
         * Javascripts files are now listed as array in index.js as
        ...
        _.each(data.assets.js, (js) => {
          // checks links to CDN
          let cdn = false;
          // checks if this it's a shared resource
          let shared = false;
          ...
          if (exists(path.join(data.module.current, js))) {
            ...
            const pathParse = path.parse(src);
            // make sure we don't overwrite a shared resource
            basename = `${pathParse.name}-${task}${pathParse.ext}`;
          }
          ...
          // Javascript file does not exists
            ...
            // 3) Check if we have an external URL
            if (isUrl.isUri(js)) {
              cdn = true;
              type === 'external';
            }
            else {
              // we fail to find the file.
          ...
          if (style === 'compressed') {
            uglifyJSResults = UglifyJS.minify(read.text(src), _.isObject(jsConfiguration.uglifyJSOptions) ? jsConfiguration.uglifyJSOptions : {});
            // minified output
            code = uglifyJSResults.code;
            // minified map output
            map = uglifyJSResults.map;
          }
    ...
    data.content = spaghetti(data);
    
    log('Transforming ' + route, 'status');
        
    // write HTML file
    write(path.join(appBuildDir(), route), minify(handlebars_template(data), htmlminifyConfiguration));

    if (get('DEBUG_LOG') === 'true') {
      write(path.join(appBuildDir(), task + '.json'), JSON.stringify(data, null, 4));
    }
